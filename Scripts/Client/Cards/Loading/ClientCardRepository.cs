...
public class ClientCardRepository : GameCardRepository<ClientSerializableCard, ClientEffect, ClientCardController>
{
	...
	public ClientGameCard? InstantiateClientAvatar(string json, ClientPlayer owner, int id, ClientGame game)
	{
		...
		ClientGameCard ConstructAvatar(ClientSerializableCard cardInfo, ClientEffect[] effects, ClientCardController normalCtrl, ClientCardController illusoryCtrl)
			=> ClientGameCard.Create(cardInfo, id, game, owner, effects, normalCtrl, illusoryCtrl, isAvatar: true);
	...
	public ClientGameCard? InstantiateClientNonAvatar(string json, IPlayer owner, int id, ClientGame game)
	{
		var card = InstantiateGameCard(SanitizeJson(json),
			(cardInfo, effects, normalCtrl, illusoryCtrl) => ClientGameCard.Create(cardInfo, id, game, owner, effects, normalCtrl, illusoryCtrl));
