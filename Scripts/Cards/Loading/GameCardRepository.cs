	...
	where TCardController : class, ICardController
{
	...
	protected delegate TGameCard ConstructCard<TGameCard>(TSerializableCard cardInfo, TEffect[] effects, TCardController normalCtrl, TCardController illusoryCtrl);
		...
		where TGameCard : GameCard
	{
		...
		var normalCtrl = GetCardController();
		var illusoryCtrl = GetCardController();
		var card = cardConstructor(cardInfo, effects.ToArray(), normalCtrl, illusoryCtrl);
		return card;
	}
