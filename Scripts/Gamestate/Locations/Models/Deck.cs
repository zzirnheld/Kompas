...
public abstract class Deck : OwnedLocationModel
{
	...
	{
		var wasInDeck = card.Location == Location.Deck && card.ControllingPlayer == Owner;

		base.PerformAdd(card, index, stackableCause);

		if (!wasInDeck) deckController.Refresh();
	}
	...
	public override void Remove(GameCard card)
	{
		...
		deck.Remove(card);
		deckController.Refresh(); //TODO don't refresh if removing from deck so can go to deck
	}
