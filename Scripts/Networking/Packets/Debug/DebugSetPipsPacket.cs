...
namespace Kompas.Server.Networking
{
	public class DebugSetPipsServerPacket : DebugSetPipsPacket, IServerOrderPacket
	{
		public Task Execute(ServerGame serverGame, ServerPlayer player)
		{
			...
			{
				Logger.Err($"Debug setting player {player.Index} pips to {numPips}");
				player.Pips = numPips;
			}
			else Logger.Err($"Tried to debug set pips of player {player.Index} to {numPips} while NOT in debug mode!");
